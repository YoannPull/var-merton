...
#
#  DRALACBN application — relaxing the satellite exogeneity assumption
...
#      source("code/dralacbn/03_dralacbn_direct_channel.R")
#
#  Outputs: output/applications/dralacbn/direct_channel/
# =============================================================================
...
dc_out_dir <- file.path("output", "applications", "dralacbn", "direct_channel")
for (sub in c("", "tables", "figures", "irf")) {
  dir.create(file.path(dc_out_dir, sub), recursive = TRUE, showWarnings = FALSE)
}

cat("\n========== DRALACBN direct-channel (control-function) exercise ==========\n")
cat("Information set: ", DC_INFO_SET, " | impulse: ", impulse_name, "\n", sep = "")

# -----------------------------------------------------------------------------
# 1. Reconstruct the DRALACBN systematic factor (identical to wrapper 02)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 4. Satellite design: best-BIC specification of the main DRALACBN run
# -----------------------------------------------------------------------------
...
if (!file.exists(best_sat_path)) {
  stop("Missing ", best_sat_path,
       "\nRun the main DRALACBN application first (run_all.R).", call. = FALSE)
}
...
# Joint orthogonality diagnostic. Regress the baseline (restricted, lambda = 0)
# satellite residual on the FULL vector of reduced-form VAR innovations at the
# posterior-median coefficients, and test their joint nullity (H0: eta is
# orthogonal to every innovation). This addresses the full-covariance question
# without requiring the individual lambda's of the included variables to be
# identified -- it is a residual-correlation test, not a structural estimate.
joint_row <- tryCatch({
  B_bar <- apply(B_kept[, , keep_m, drop = FALSE], c(1, 2), stats::median)
  U_med <- Y_t - X_t %*% B_bar                 # n_eff x n_var reduced-form innovations
  colnames(U_med) <- variables
  U_ov  <- U_med[row_map, , drop = FALSE]       # aligned to the satellite rows
  eta_med <- stats::residuals(stats::lm(
    stats::as.formula(paste("Y ~", paste(sat_terms, collapse = " + "))),
    data = as.data.frame(sat_df)))
  jdf   <- data.frame(eta = as.numeric(eta_med), U_ov, check.names = FALSE)
  fit_j <- stats::lm(eta ~ ., data = jdf)
  smj   <- summary(fit_j)
  fst   <- smj$fstatistic
  data.table(
    test = "joint orthogonality: eta ~ full reduced-form innovations (posterior median)",
    F_stat = unname(fst[1]), df1 = unname(fst[2]), df2 = unname(fst[3]),
    p_value = stats::pf(fst[1], fst[2], fst[3], lower.tail = FALSE),
    r_squared = smj$r.squared, n_obs = nrow(jdf)
  )
}, error = function(e) data.table(
  test = "joint orthogonality", F_stat = NA_real_, df1 = NA_real_,
  df2 = NA_real_, p_value = NA_real_, r_squared = NA_real_,
  n_obs = NA_integer_, note = conditionMessage(e)))
fwrite(joint_row, file.path(dc_out_dir, "tables", "orthogonality_joint_test.csv"))
cat("Joint orthogonality test (eta vs full innovations): F = ",
    formatC(joint_row$F_stat, format = "f", digits = 2), " on (",
    joint_row$df1, ", ", joint_row$df2, ") df, p = ",
    formatC(joint_row$p_value, format = "f", digits = 3), "\n", sep = "")
...
saveRDS(
  list(
    ...
    joint_orthogonality_test = joint_row,
...
cat("\\caption{Direct geopolitical channel in the satellite equation (DRALACBN)}\n", file = con)
cat("\\label{tab:dralacbn_direct_channel}\n", file = con)
...
readme <- c(
  ...
  "",
  "Generated by code/dralacbn/03_dralacbn_direct_channel.R (standalone; not in run_all.R).",
  "",
  "Contents:",
