# =============================================================================
#  code/extra/coralacbn/04_coralacbn_direct_channel_paper_outputs.R
#
#  Publication-quality figures, tables and clean CSVs for the direct-channel
#  (control-function) exercise. Reads the results of
#  code/extra/coralacbn/03_coralacbn_direct_channel.R and writes paper-ready outputs.
#
#  STANDALONE: modifies nothing in the existing pipeline; not in run_all.R.
#  Run from the repository root AFTER 03_coralacbn_direct_channel.R:
#      source("code/extra/coralacbn/04_coralacbn_direct_channel_paper_outputs.R")
#
#  Outputs: output/applications/coralacbn/direct_channel/paper/
...
res_path <- file.path("output", "applications", "coralacbn",
                      "direct_channel", "direct_channel_results.rds")
if (!file.exists(res_path)) {
  stop("Missing ", res_path,
       "\nRun code/extra/coralacbn/03_coralacbn_direct_channel.R first.", call. = FALSE)
}
res <- readRDS(res_path)

paper_dir <- file.path("output", "applications", "coralacbn", "direct_channel", "paper")
